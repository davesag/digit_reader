...
class DigitReader {
  ...
  constructor(options) {
    ...
    // layerDefinitions.push({type:'input', out_sx: 128, out_sy: 128, out_depth: 4});
    // layerDefinitions.push({type:'conv', sx: 5, filters: 16, stride: 1, pad: 2, activation: 'relu'});
    // layerDefinitions.push({type:'pool', sx: 8, stride: 2});
    // layerDefinitions.push({type:'conv', sx: 5, filters: 20, stride: 1, pad: 2, activation: 'relu'});
    // layerDefinitions.push({type:'pool', sx: 8, stride: 2});
    // layerDefinitions.push({type:'softmax', num_classes: 10});
    ...
    // let softmax = DigitReader.mergeDefaults(options, 'softmax');
    // this.length = 10; // softmax.num_classes;
    // layerDefinitions.push(softmax);
